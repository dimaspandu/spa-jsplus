...
export default function transpileImportTokensToCJS(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // ===============================================================
      ...
      // assert { type: "json" };
      // with { type: "css" };
      const tZ = tokens[scopeEndIdx];
      if (tZ && (tZ.value === "assert" || tZ.value === "with")) {
        skippedIndex[scopeEndIdx] = 1;
        const endIndex = getObjectLiteralsEndIndex(tokens, scopeEndIdx + 1);
        for (let j = scopeEndIdx + 1; j <= endIndex; j++) {
          skippedIndex[j] = 1;
        }
        skipIfSemi(endIndex + 1);
      }
