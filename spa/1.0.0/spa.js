...
(function(global) {
  ...
  function setHistoryStateSpa(spa, method, originPath, requestPath) {
    var basePath;
    if (spa.options.hashtag) {
      basePath = global.location.pathname + spa.options.dividerPath;
    } else {
      basePath = spa.options.baseUrl + "";
    }

    var stateObj = { origin: originPath, request: requestPath };
    var url = basePath + requestPath;

    // use replace or push (default push)
    if (method === "replace" && global.history && global.history.replaceState) {
      ...
      // very old browsers: fallback by changing location hash or href (best-effort)
      try {
   ...
   *
   * The constructor will normalize options for ES5-safe use.
   */
  function Spa(options) {
    // normalize options argument to object (ES5-safe)
    options = options || {};

    var baseUrlOption = typeof options.baseUrl === "undefined" ? "" : String(options.baseUrl);
    baseUrlOption = safeTrimRightSlash(baseUrlOption);

    var dividerPathOption = typeof options.dividerPath === "undefined" ? "#" : String(options.dividerPath);
    var hashtagOption = typeof options.hashtag === "undefined" ? true : !!options.hashtag;
    var hostdomOption = typeof options.hostdom === "undefined" ? document.body : options.hostdom;
    ...
    if (!this.options.hashtag && (!this.options.baseUrl || this.options.baseUrl.length === 0)) {
      if (global && global.console && global.console.error) {
        global.console.error("There is no {baseUrl} set in instance of spa arguments options while disabling hashtag options.");
      }
    }
    ...
    if (!this.options.hashtag && this.options.baseUrl.length > 0) {
      if (this.options.dividerPath !== "#" && this.options.dividerPath !== this.options.baseUrl) {
        if (global && global.console && global.console.warn) {
          global.console.warn("Please set {dividerPath} same as {baseUrl} while disabling hashtag options.");
        }
      }
      // when hashtag disabled, use baseUrl as divider
