...
export function setNavigationSpa(spa, method, originPath, requestPath, params, query) {
  ...
  if (pipe.context === null) {
    ...
    // Override update container content
    /**
     * setContainer
     *
     * Dynamically overrides the current container at runtime.
     *
     * Typical usage:
     * - Unlike `ctx.container`, which is usually declared once when defining
     *   a reactor, `ctx.setContainer` allows you to replace the container
     *   conditionally while the route is already being processed.
     *
     * Example:
     *   In `/search/{query}`, if the query equals "nowhere",
     *   we can override the container with a "not found" message:
     *
     *     ctx.setContainer(() => (`
     ...
     *
     * Behavior:
     * - Accepts a DOM node, a string of HTML, or a function returning either.
     * - Clears the content of `hostdom` and injects the new content immediately.
     * - Works as a bridge between dynamic route logic and the actual DOM.
     */
    context.setContainer = function(value) {
      const container = isFunction(value) ? value() : value;
